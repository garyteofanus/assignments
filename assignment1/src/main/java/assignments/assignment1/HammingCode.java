...
public class HammingCode {
   ...
   */
    public static String encode(String data) {
        ...
        for (int i = 1; i < (r + m); i *= 2) {
            ...
            // The use of temp to hold the bits value, to be calculated for determining the
            // actual value of parity bits
            // Counter variable will keep track on the number of bits that needs to be check and
            // skipped, the status is used to determined if the bit should be checked of skipped
   ...
   */
    public static String decode(String code) {
        ...
        int wrongBit = 0;
        // The logic used in the decode function is similiar to encode, the difference is that
        // there is no need for skipping the 'x' value.
        ...
        // If there are no wrongBit then there is no need to replace the wrong bit with the true
        // value, in case of there is a wrong bit, the bit value is switched using xor operation.
        String correct;
        if (wrongBit == 0) {
            correct = code;
        } else {
            correct =
                code.substring(0, wrongBit - 1)
                    + Character.toString(code.charAt(wrongBit - 1) ^ 1) + code
                        .substring(wrongBit);
        }
        StringBuilder result = new StringBuilder();
        int a = 0;
